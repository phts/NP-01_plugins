...
peppymeterbasic.prototype.getI18nFile = function (langCode) {
  ...
  if (
    i18nFiles.some(function (i18nFile) {
      return i18nFile === langFile;
    })
  ) {
...
peppymeterbasic.prototype.onStart = function () {
  ...
  self.commandRouter
    ...
    .then(function (e) {
      const pipeDefer = libQ.defer();
      exec(
        '/usr/bin/mkfifo /tmp/basic_peppy_meter_fifo' + '; /bin/chmod 666 /tmp/basic_peppy_meter_fifo',
        {uid: 1000, gid: 1000},
        function (error, stdout, stderr) {
          if (error) {
...
peppymeterbasic.prototype.getUIConfig = function () {
  ...
  self.commandRouter
    ...
    .then(function (uiconf) {
      let meterfolder;
      const showsize = self.config.get('showsize');
      const autosize = self.config.get('auutosize');
        ...
        fs.readFile(meterfolder + valuescreen + '/meters.txt', function (err, idata) {
          ...
          try {
            ...
            for (i = 0; i < result.length; i++) {
              const preparedresult = result[i].split('.')[0];
...
peppymeterbasic.prototype.savepeppy = function (data) {
  ...
  if (screensize === '320x240' || screensize === '480x320' || screensize === '800x480' || screensize === '1280x400') {
    ...
    const sizef = autovalue[0];

    const size = sizef; // .slice(0, -1)
    // Split the string by comma and convert each element to integer
    const sizen = sizef.split(',').map(function (value) {
    ...
    const truex = typeof myNumberx === 'number' && isFinite(myNumberx);
    const truey = typeof myNumbery === 'number' && isFinite(myNumbery);
    const trues = typeof myMeterSize === 'number' && isFinite(myMeterSize);
  ...
  const storedmetersize = self.config.get('metersize');
...
peppymeterbasic.prototype.buildasound = function () {
  ...
  try {
    fs.readFile(__dirname + '/peppy_in.peppy_out.6.conf.tmpl', 'utf8', function (err, data) {
      ...
      fs.writeFile(
        '/data/plugins/user_interface/peppymeterbasic/asound/peppy_in.peppy_out.6.conf',
        conf1,
        'utf8',
        function (err) {
...
peppymeterbasic.prototype.savepeppyconfig = function () {
  ...
  try {
    fs.readFile(__dirname + '/config.txt.tmpl', 'utf8', function (err, data) {
      ...
      const autosize = self.config.get('autosize');

      let screensize = self.config.get('screensize');
      if (screensize == '320x240' || screensize == '480x320' || screensize == '800x480' || screensize == '1280x400') {
      ...
      const debuglog = self.config.get('debuglog');
      if (debuglog) {
        var debuglogd = 'True';
      ...
      fs.writeFile(
        '/data/plugins/user_interface/peppymeterbasic/BasicPeppyMeter/config.txt',
        conf1,
        'utf8',
        function (err) {
          if (err) defer.reject(new Error(err));
...
peppymeterbasic.prototype.updatelist = function (data) {
  ...
  const defer = libQ.defer();
  const destpath = " '/data/plugins/user_interface/peppymeterbasic";
  // self.config.set('importeq', namepath)
  const toDownload = path + '/' + name + "'";
