...
ControllerSpotify.prototype.getUIConfig = function () {
    ...
    self.commandRouter
        ...
        .then(function (uiconf) {
            ...
            uiconf.sections[2].content[2].value.label = self.getLabelForSelect(
                uiconf.sections[2].content[2].options,
                icon
            );
...
ControllerSpotify.prototype.setSpotifyDaemonVolume = function (volume) {
    ...
    if (apiCallsCounter >= apiCallsLimit) {
        self.debugLog(
            'VOLUME LIMITER, API calls reached: ' + apiCallsCounter + ' calls in ' + apiCallsTimespan + ' ms'
        );
    } else {
...
ControllerSpotify.prototype.getMyPlaylists = function (curUri) {
    ...
    self.spotifyCheckAccessToken().then(function (data) {
        ...
        self.spotifyApi.getUserPlaylists(self.loggedInUserId).then(
            function (results) {
