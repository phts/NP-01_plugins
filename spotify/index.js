...
ControllerSpotify.prototype.onStart = function () {
  ...
  this.checkOldSpotifyConnectPlugin()
    .then(() => {
      // SpotifyWebApi
...
ControllerSpotify.prototype.getUIConfig = function () {
  ...
  self.commandRouter
    ...
    .then(function (uiconf) {
      ...
      uiconf.sections[2].content[8].value.label = self.getLabelForSelect(
        uiconf.sections[2].content[8].options,
        deviceIconConf
      );
...
ControllerSpotify.prototype.VolspotconnectServiceCmds = async function (cmd) {
  ...
  const { stdout, stderr } = await exec(`/usr/bin/sudo /bin/systemctl ${cmd} volspotconnect.service`, {
    uid: 1000,
    gid: 1000,
  });
  if (stderr) {
...
ControllerSpotify.prototype.volspotconnectDaemonConnect = function (defer) {
  ...
  this.SpotConn.on(this.Events.Metadata, (meta) => {
    ...
    if (
      currentTrackContext &&
      currentTrackContext.trackId !== undefined &&
      meta !== undefined &&
      meta.track_id !== undefined &&
      currentTrackContext.trackId === meta.track_id &&
      currentTrackContext.isConnect !== undefined
    ) {
      if (!this.isStopping) {
        this.pushState();
      }
      ...
      setTimeout(() => {
        this.isPlaybackFromConnectDevice(meta.track_id)
          .then((isConnect) => {
            if (isConnect) {
...
ControllerSpotify.prototype.init = async function () {
  ...
  try {
    ...
    // So be warned...
    this.commandRouter.sharedVars.registerCallback('alsa.outputdevice', this.rebuildRestartDaemon.bind(this));
    this.commandRouter.sharedVars.registerCallback('alsa.outputdevicemixer', this.rebuildRestartDaemon.bind(this));
    this.commandRouter.sharedVars.registerCallback('alsa.device', this.rebuildRestartDaemon.bind(this));
    this.commandRouter.sharedVars.registerCallback('system.name', this.rebuildRestartDaemon.bind(this));
  } catch (e) {
...
ControllerSpotify.prototype.createConfigFile = async function () {
  ...
  let initvol = '0';
  var volumestart = this.commandRouter.executeOnPlugin(
    'audio_interface',
    'alsa_controller',
    'getConfigParam',
    'volumestart'
  ...
  var outdev = 'volumio';
  var volcuve = this.commandRouter.executeOnPlugin(
    'audio_interface',
    'alsa_controller',
    'getConfigParam',
    'volumecurvemode'
...
ControllerSpotify.prototype.checkOldSpotifyConnectPlugin = function () {
  ...
  if (isOldPluginEnabled) {
    ...
    self.commandRouter
      .disablePlugin({ category: 'music_service', plugin: 'volspotconnect2' })
      .then(() => {
        var responseData = {
          title: 'SPOTIFY PLUGIN',
          message:
            'WARING! The Spotify plugin might not work properly, since you have both volspotconnect2 and Spotify plugin installed. ' +
