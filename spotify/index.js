...
let seekTimer;
let restartTimeout;
let playbackStartWatchdog;
const playbackStartTimeout = 10000;
let playbackStartConfirmed = false;
let deviceAuthInProgress = false;
let deviceAuthModal;
let deviceAuthCancelled = false;
...
ControllerSpotify.prototype.getUIConfig = function () {
  ...
  self.commandRouter
    ...
    .then(function (uiconf) {
      ...
      const normalisationPregainValue = self.config.get('normalisation_pregain', '1.0');
...
ControllerSpotify.prototype.clearAddPlayTrack = function (track) {
  ...
  return self.hasActiveDaemonSession().then((hasSession) => {
    ...
    return superagent
      .post(spotifyLocalApiEndpointBase + '/player/play')
      .accept('application/json')
      .send({uri: track.uri})
      .then((results) => {})
      .catch((error) => {
...
ControllerSpotify.prototype.getDaemonPairingPrompt = function () {
  ...
  return superagent
    .get(spotifyLocalApiEndpointBase + '/auth/code')
    .accept('application/json')
...
ControllerSpotify.prototype.hasActiveDaemonSession = function () {
  ...
  return superagent
    .get(spotifyLocalApiEndpointBase + '/status')
    .accept('application/json')
...
ControllerSpotify.prototype.abortPlayback = function (reason) {
  ...
  const stateMachine = self.commandRouter.stateMachine;
  if (stateMachine) {
...
ControllerSpotify.prototype.startAuthorization = function (data) {
  ...
  const release = function (message) {
...
ControllerSpotify.prototype.authorizeBrowsing = function (data) {
  ...
  const performerUrl = self.buildPerformerUrl(data);
  if (!performerUrl) {
  ...
  self.shortenUrl(performerUrl).then(function (short) {
    ...
    self.pushAuthModal(
      self.getI18n('PAIRING_TITLE'),
      self.buildSignInMessage(short),
...
ControllerSpotify.prototype.buildSignInMessage = function (short) {
  ...
  return self.joinModalLines([self.getI18n('STEP_ONE_TODO'), '', short.url]) + self.modalQr(short.qr);
};
...
ControllerSpotify.prototype.shortenUrl = function (url) {
  ...
  const attempt = function (retriesLeft) {
    superagent
      .post('https://volm.io/shorten')
      .send({url: url})
      .accept('application/json')
      .timeout({response: 5000, deadline: 8000})
      .then(function (results) {
        const body = (results && results.body) || {};
  ...
  return self
    ...
    .then(function (prompt) {
      ...
      self.shortenUrl(prompt.url).then(function (short) {
        ...
        const withQr = Object.assign({}, prompt, {qr: short.qr});
...
ControllerSpotify.prototype.buildPerformerUrl = function (data) {
  ...
  const device = self.commandRouter.executeOnPlugin('system_controller', 'volumiodiscovery', 'getThisDevice');
  if (!device || !device.host) {
    self.logger.error('Cannot build the Spotify OAuth url: this device has no reachable host');
    return undefined;
  }

  const redirectUri = new URL(device.host + '/api/v1/oauth');
  ...
  redirectUri.searchParams.set('plugin_url', device.host + '/plugin/' + data.plugin.replace('/', '-'));

  const performerUrl = new URL(data.performerUrl);
...
ControllerSpotify.prototype.buildAuthMessage = function (prompt) {
  const self = this;
  const lines = [];
  const signedInAs = self.config.get('logged_user_id', '');
  ...
  if (!prompt) {
    ...
    lines.push(
      self.getPlaybackAuthorization().authorized ? self.getI18n('STEP_TWO_DONE') : self.getI18n('PAIRING_CONTACTING')
    );
    return self.joinModalLines(lines);
  }

  lines.push(
    self.getI18n('STEP_TWO_TODO'),
    '',
    prompt.url,
    '',
  ...
  const expiry = new Date(prompt.expiresAt);
  if (expiry.getTime() > Date.now()) {
    lines.push(
      self.getI18n('PAIRING_EXPIRES') +
        ' ' +
        ('0' + expiry.getHours()).slice(-2) +
        ':' +
        ('0' + expiry.getMinutes()).slice(-2)
    );
  }
...
ControllerSpotify.prototype.applyAccountSectionState = function (uiconf) {
  ...
  const signedIn = self.loggedInUserId !== undefined && self.config.get('refresh_token', '') !== '';
  const playback = self.getPlaybackAuthorization();
  const authorized = signedIn && playback.authorized;

  const authorizeButton = self.findUiElement(uiconf, 1, 'authorize');
  authorizeButton.hidden = authorized;
  self.applyManifestOauthAction(authorizeButton);

  const revokeButton = self.findUiElement(uiconf, 1, 'deauthorize');
...
ControllerSpotify.prototype.findUiElement = function (uiconf, section, id) {
  const content = uiconf.sections[section].content;
...
ControllerSpotify.prototype.applyManifestOauthAction = function (authorizeButton) {
  ...
  const call = (authorizeButton.onClick.data || {}).data || {};
...
ControllerSpotify.prototype.getActiveUiName = function () {
  ...
  try {
    const active = fs.readJsonSync('/data/active_volumio_ui');
  ...
  try {
    const installed = fs.readJsonSync('/volumio/volumioUisList.json');
...
ControllerSpotify.prototype.getPlaybackAuthorization = function () {
  ...
  try {
    const state = JSON.parse(fs.readFileSync(credentialsPath, {encoding: 'utf8'}).toString());
...
ControllerSpotify.prototype.refreshAccessToken = function () {
  ...
  if (refreshToken !== 'none' && refreshToken !== null && refreshToken !== undefined) {
    superagent
      .post('https://oauth-performer.prod.vlmapi.io/spotify/accessToken')
...
ControllerSpotify.prototype.listWebNew = function (curUri) {
  ...
  self.spotifyCheckAccessToken().then(function (data) {
    const spotifyDefer = self.spotifyApi.getNewReleases({limit: 50});
    spotifyDefer.then(
      function (results) {
        const response = {
          navigation: {
...
ControllerSpotify.prototype.listWebAlbum = function (curUri) {
  ...
  const spotifyDefer = self.getAlbumTracks(uriSplitted[2], {limit: 50});
  spotifyDefer.then(function (results) {
    const response = {
      navigation: {
        prev: {
    ...
    const albumInfo = self.getAlbumInfo(uriSplitted[2]);
...
ControllerSpotify.prototype.listArtistTracks = function (id) {
  ...
  const spotifyDefer = self.getArtistTopTracks(id);
...
ControllerSpotify.prototype.listArtistAlbums = function (id) {
  ...
  const spotifyDefer = self.spotifyApi.getArtistAlbums(id);
  spotifyDefer.then(function (results) {
    const response = [];
    for (const i in results.body.items) {
      const album = results.body.items[i];
      response.push({
        service: 'spop',
        type: 'folder',
        title: album.name,
...
ControllerSpotify.prototype.getArtistTracks = function (id) {
  ...
  spotifyDefer
    ...
    .then(function (data) {
      const spotifyDefer = self.getArtistAlbumTracks(id);
      spotifyDefer.then(function (results) {
        const response = data;
        for (const i in results) {
...
ControllerSpotify.prototype.getArtistAlbumTracks = function (id) {
  ...
  spotifyDefer
    ...
    .then(function (albums) {
      const spotifyDefer = self.spotifyApi.getAlbums(albums);
      spotifyDefer.then(function (data) {
        const results = data;
        const response = [];
        for (const i in results.body.albums) {
          const album = results.body.albums[i];
          for (const j in album.tracks.items) {
            const track = album.tracks.items[j];
            if (self.isTrackAvailableInCountry(track)) {
              response.push({
                service: 'spop',
                type: 'song',
...
ControllerSpotify.prototype.getArtistAlbums = function (artistId) {
  ...
  self.spotifyCheckAccessToken().then(function (data) {
    const spotifyDefer = self.spotifyApi.getArtistAlbums(artistId);
    spotifyDefer.then(function (results) {
      const response = [];
      for (const i in results.body.items) {
        const album = results.body.items[i];
...
ControllerSpotify.prototype.getAlbumTracks = function (id) {
  ...
  self.spotifyCheckAccessToken().then(function (data) {
    const spotifyDefer = self.spotifyApi.getAlbum(id);
    spotifyDefer.then(
      function (results) {
        const response = [];
        const album = results.body.name;
        const albumart = results.body.images[0].url;
...
ControllerSpotify.prototype.getArtistTopTracks = function (id) {
  ...
  self.spotifyCheckAccessToken().then(function (data) {
    const spotifyDefer = self.spotifyApi.getArtistTopTracks(id, 'GB');
    spotifyDefer.then(function (results) {
      const response = [];
      for (const i in results.body.tracks) {
        let albumart = '';
        const track = results.body.tracks[i];
...
ControllerSpotify.prototype.searchAlbumByName = function (albumName) {
  ...
  const spotifyDefer = self.spotifyApi.search(albumName, ['album']);
