...
ControllerSpotify.prototype.onSpotifyVolumeChange = function (volume) {
  ...
  if (!this.config.get('volume_control')) {
    self.logger.info('Spotify volume control is disabled by settings. Force set to 100.');
    if (volume !== 100) {
...
ControllerSpotify.prototype.onVolumioVolumeChange = function (volume) {
  ...
  if (!this.config.get('volume_control')) {
    self.logger.info('Spotify volume control is disabled by settings. Volume change was ignored.');
    return;
  }
  ...
  if (curUri.startsWith('spotify')) {
    if (curUri == 'spotify') {
      ...
      self
        ...
        .then((res) => response.resolve(res))
        .catch((e) => response.reject(e));
    } else if (ROUTE_ARTIST_TOP_TRACKS.test(curUri)) {
      const artistId = curUri.match(ROUTE_ARTIST_TOP_TRACKS)[1];
      response = libQ.defer();
      self
...
ControllerSpotify.prototype.listArtistTopTracks = async function (artistId) {
  const items = await this.getArtistTopTracks(artistId);
  const prefix = items.length ? `${items[0].artist}: ` : '';
  return {
    navigation: {
      prev: {
      ...
      lists: [
        {
          title: `${prefix}${this.getI18n('TOP_TRACKS')} (${items.length})`,
          availableListViews: ['list'],
          items,
        },
      ],
...
ControllerSpotify.prototype.getArtistTopTracks = async function (artistId) {
  await this.spotifyCheckAccessToken();
  try {
    const {
      body: {tracks},
